...
watch(encodedData, () => {
  ...
  gameStore.redirectLink = encodedData.value?.link || 'https://levelx.top/tee67a256'
}, {immediate: true})

const scaleValue = computed(() => {
  if (!gameRef.value) return 'scale(1)'

  const targetWidth = Screen.width > 800 ? Screen.width * 0.50 : Screen.width * 0.8
...
.logo {
  @apply tw-absolute tw-top-[20px] tw-left-0 tw-h-[40px] tw-cursor-pointer;
  translate: calc(100% - 150px) 0;
